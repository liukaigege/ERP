 ...
 * @version: 
 * @Author: 韦剑
 * @Date: 2021-07-12 09:36:54
 * @LastEditors: 朱涛
 * @LastEditTime: 2021-08-22 23:26:47
-->
<template>
	<div class="tableView">
		<table>
			...
			<tbody>
				<template v-for="(scoped, keys) in filterData" :key="keys">
					<tr class="tbHead">
						...
						<td :colspan="getHeaderCol(columns.length, scoped,keys)" >
							<slot name="extraHeader" :row="scoped" ></slot>
						</td>
					</tr>
					<tr class="list" :class="{ showlist: ifShowList }" v-for="(scope, key) in scoped[fieldName]" :key="key">
						<td v-if="checkboxType" :colspan="checkboxType == 1 ? 2 : 1" :style="checkboxType == 1 ?`width:${columns[0]?.width}` : '80px'"  :valign="valign(columns[0]?.key)">
						...
						<template v-for="(item, index) in columns" :key="index">
							<td  v-if="showTd(item.key, scoped, scope, keys, index,key)" :rowspan="getRowSpan(item.key, scoped, scope, keys, index,scoped[fieldName],key)" :valign="valign(item.key)" >
...
<script>
	import { computed, onBeforeUnmount, reactive, toRefs, watch, nextTick, onActivated, onDeactivated } from 'vue'
	import { startIndex, endIndex, setData, initVirtual } from './js/virtualScroll'
	...
	export default {
		...
		setup(props, context) {
			...
			function scroll(e) {
				...
				setData(props.tableData.length)
			}
			...
			watch(() => props.tableData, (newValue,oldValue) => {
                if(newValue.length == oldValue.length) {
                    // 判断数据相似度
                    if(props.checkedKey) {
                        let flag =  newValue.some(item => item[props.checkedKey] == oldValue[0][props.checkedKey])
                        if(!flag) initVirtual()
                    }
                }else {
                    initVirtual()
			...
			const getRowSpan = (name, parent, children, faIndex, index, orderList, chIndex) => {
				...
				if (props.isMearg && name == 'options') {
					...
					if (props.mearge[parent[meargeKey]]) {
						...
						let obj = props.mearge[parent[meargeKey]]
						span = obj.rowSpan + obj.packgeNum-1
					}
				}
			...
			const getHeaderCol = (length, parent,keys) => {
				const {mearge,isMearg,meargeKey} = props
				const {packageId} = parent
				if (props.isMearg) {
					return length - headRowSpan(parent[meargeKey],packageId)
				} else {
					return length + 1
				}
				function headRowSpan(val,id){
					let bole = 1
					let x = mearge[val]
					 if(x && x.packageId == id){
							 bole = 0
					 ...
					 return bole
				}  // 控制头部是否合并到底 如果当前包裹的id存在于 合并数据中即可合并到底 否则不可合并到底
			...
			const valign = val =>{
					if(!props.isValign) return 'middle'
				  let list = ['orderGoods','sendGoods']
					if(list.indexOf(val) !== -1) return 'top'
