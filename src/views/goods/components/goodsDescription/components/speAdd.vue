      ...
      <div class="container">
        ...
        <div class="speList">
          ...
          >
            ...
            <p :title="item.valCn || item.valEn">{{ showName(item) }}</p>
          </div>
        </div>
      </div>
      <template #footer>
...
export default {
  ...
  setup(props, context) {
    ...
    watch(
      ...
      a => {
        console.log(a)
        if (a.length == props.query.length) data.isCheckedAll = true
    ...
    const dialogHandOk = () => {
      ...
      data.activeIds.map(item => {
        let index = props.query.findIndex(v => v.id == item)
        if (index !== -1) selectList.push(props.query[index])
      })
    ...
    function showIndeter() {
      if (
        ...
        data.activeIds?.length < props.query?.length
      )
        return true
      return false
    }
