...
<template>
  <div>
    ...
    >
      ...
      <div class="imgBox">
       <div class="imgItem" v-if="imgList['1'].length == 0 && activeName == '1'"> 
              <UploadImg 
                ref="uploadImg"
                :data="[]"
                id="uploadImg"
        ...
        <draggable
          ...
          <div style="display: flex;">
            ...
            <div :class=" selectedList.indexOf(element.pictureId) !== -1 ? 'imgItemActive imgItem ' : 'imgItem'  " @click="selected(element)" >
...
import skuItemHook from "../hooks/skuItem.js";
  export default {
      ...
      const dialogHandOk = () => {
           ...
           if(data.selectedList?.length >0){
             ...
             data.imgList[props.activeName].map(item=>{
                if(data.selectedList.indexOf(item.pictureId) !==-1) imgArr.push(item)
             })
             
             emit('selectImg',{query:props.skuQuery,url:imgArr})
              cancel()
           }else{
    ...
    function pushNewImg(){
       let {newList} = getActiveList()
        newList?.forEach(item=>{
          ...
          props.selectImageData.attrImages.push(item)
        })
        data.newList = []
    }
    ...
    function init(){
      const {attrImages,goodsImages} =   props.selectImageData
