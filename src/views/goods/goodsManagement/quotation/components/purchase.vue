<template>
  <div>
    <filter-form :form="filterForm" type="purchase" @search="searchAndRefresh"></filter-form>
    <div class="padding-flex-wrap justify-between mb-15">
      <div>
        <el-button v-permission="'purchaseBatch:btn'"  type="primary" @click="batchQuote('PurchaseBatch')">批量报价</el-button>
        <el-button v-permission="'purchaseBatchReject:btn'"  type="primary" @click="batchReject">批量打回</el-button>
        <el-button v-permission="'quotationCombine:btn'"  type="primary" @click="goCombine('QuotationCombine')">合并商品</el-button>
        <el-button v-permission="'quotationRemark:btn'"  type="primary" @click="openRemarkDialog">备注</el-button>
        <el-button v-permission="'quotationImport:btn'" type="primary"  @click="importDialogVisible = true">导入采购信息</el-button>
      </div>
      <el-button v-permission="'quotationExport:btn'" @click="exportGoods('purchase')" type="primary" >导出采购信息</el-button>
    </div>
    <el-card shadow="never" class="none-border-card">
      <div class="purchase-table-view">
        <vxe-table ref="tableRef" :loading="tableLoading" :scroll-y="{enabled: false}" border :data="tableData" :span-method="colspanMethodP">
            ...
            <template #default="{ row, rowIndex }">
              <div v-if="rowIndex % 2 === 0" class="goods-id-div">
                <div class="goods-id">
                  ...
                  <el-popover placement="top" :width="800" trigger="hover" v-for="(item, index) in row.remarkList" :key="index" @after-enter="showRemarkDetail(row.id, item.typeId)">
                    <template #reference>
          ...
          <vxe-column field="sourceLabel" title="来源" align="center">
            <template #default="{ row }">
              <div>{{row.sourceLabel}}</div>
              <a v-if="row.collectionUrl" :href="row.collectionUrl" target="_blank">参考链接</a>
            </template>
          </vxe-column>
          <vxe-column title="操作员" align="center">
            <template #default="{ row }">
              <div v-permission="'purchase:word:developer'">开发人：{{row.createUserName}}</div>
              <div v-permission="'purchase:word:applicant'">申请人：{{row.applyUserName}}</div>
              <div v-if="row.refuseType && row.refuseType === 3">打回人：{{row.refuseUserName}}</div>
            </template>
          </vxe-column>
          <vxe-column field="createTime" title="时间" align="center" width="240">
            <template #default="{ row }">
              <div>开发时间：{{row.createTime}}</div>
              <div :class="(Date.now() - new Date(row.applyTime).getTime()) > 64800000 ? 'color-danger' : ''">申请时间：{{row.applyTime}}</div>
              <div v-if="row.refuseType && row.refuseType === 3">打回时间：{{row.refuseTime}}</div>
            </template>
          </vxe-column>
          <vxe-column field="authTypeLabel" title="查看权限" align="center"></vxe-column>
          <vxe-column field="deptName" title="开发部门" align="center"></vxe-column>
          <vxe-column title="操作" align="center" width="100" v-if="authPath('purchaseQuote:btn')">
            <template #default="{ row }">
              <div>
                <el-button type="text" @click="singleQuote('PurchaseQuote', row.id)">采购报价</el-button>
              </div>
              <div>
                <el-button type="text" v-if="row.refuseType && row.refuseType === 3" @click="showRefuseReason(row.offerRefuseReson)">打回原因</el-button>
              </div>
            </template>
          </vxe-column>
        </vxe-table>
    ...
    <return-dialog v-if="returnDialogVisible" :selections="batchIds" reasonType="purchase" @close="closeDialog"></return-dialog>
    <import-dialog type="purchase" v-if="importDialogVisible" @close="closeDialog"></import-dialog>
    <remark v-if="remarkDialogVisible" @closeEvent="closeRemarkDialog" :goodsIds="selections"></remark>
  </div>
</template>
...
export default {
  emits: ['refresh'],
  components: { ReturnDialog, FilterForm, Remark, ImportDialog },
  setup(props, ctx) {
    const state = reactive({
      filterForm: {
        goodsStatus: 2, // 1未报价 2待采购报价 3报价打回 4待物流报价 6已报价
        timeType: null, // 时间种类
        start_end: [], // 开始时间 结束时间
        enableCustom: '', // 商品类型
        goodsSrc: '0', // 商品来源
