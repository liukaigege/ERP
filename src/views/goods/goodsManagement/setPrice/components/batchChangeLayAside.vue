<template>
	<el-dialog title="批量移入搁置" v-model="ifShow" :width="640" custom-class="myDialog" :close-on-click-modal="false" :destroy-on-close="true" @close="closeEvent">
		<div class="contentView">
			<el-form  :model="form" :rules="rules" ref="formRef">
        <i class="el-icon-warning"></i>
        <span class="text">确认将勾选的商品移入搁置？</span>
				<el-form-item label="备注:" style="margin-top: 24px" prop="remark">
...
<script>
  ...
  import { batchAddLay } from '@/api/goods/quotedPrice.js'
	...
	export default {
      ...
      // 确认回调
          ...
          if (valid) {
            state.loading = true
            batchAddLay(state.form).then(res => {
              state.loading = false
							if (res.code == 200) {
