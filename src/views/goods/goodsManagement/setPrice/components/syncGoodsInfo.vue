<template>
	<el-dialog title="强制同步商品信息" v-model="ifShow" :width="640" custom-class="myDialog" :close-on-click-modal="false" :destroy-on-close="true" @close="closeEvent">
		<div class="contentView">
			<el-form  :model="form" :rules="rules" ref="formRef">
        <i class="el-icon-warning"></i>
        <span class="text">确认同步商品信息到已刊登的商品？</span>
				<el-form-item label="同步备注:" style="margin-top: 24px" prop="remark">
...
<script>
  ...
  import { ElMessage } from 'element-plus'
	import { batchSyncInfo } from '@/api/goods/quotedPrice.js'
	export default {
		props: {
			goodsIds: {
				type: Array,
      ...
      // 确认回调
          ...
          if (valid) {
            state.loading = true
            batchSyncInfo(state.form).then(res => {
              state.loading = false
							if (res.code == 200) {
