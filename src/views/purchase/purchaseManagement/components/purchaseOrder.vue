	...
	<div class="container">
		...
		<div class="table">
			...
			>
				...
				<vxe-column field="goods" title="商品信息" width="400">
				...
				<vxe-column field="pairInfo" title="供应商商品信息" width="520">
					<template #default="{ row }">
						<div class='box'>
				...
				>
					...
					<template #default="{ row }">
						<p :style="computedSinglePrice(row) ? 'color :red' : ''">
							{{ (row.signlePrice - 0).toFixed(2) }}
						</p>
					</template>
				</vxe-column>

				<vxe-column field="num" title="数量" width="120">
					<template #default="{ row }">
						<vxe-input
				...
				<vxe-column field="yprice" title="应付" width="120">
					<template #default="{ row }">
						￥{{ (row.quantity * row.signlePrice - 0).toFixed(2) }}
					</template>
				</vxe-column>
		...
		<Edit v-if="editShow" @editBack="editBack" :selectedList="selectedList" :remarkList="remarkList" :accounts="accounts"/>
	</div>
</template>
...
const tableRules = {
	...
	// pairInfo: [{ validator: valitorPairInfo, trigger: 'blur' }]
}
...
function valitorSinglePrice({ row }) {
	...
	if (type === 'item') {
		...
		if (Number(signlePrice) > Number((purchasePrice - 0 + 3).toFixed(2)))
...
function getOrganization(){
	getOrganizationTree().then(res=>{
		...
		if(res.code === 200){
			...
			console.log(data.oraganziationList)
		}
	})
...
function computedSinglePrice(row) {
	...
	// let price = lastPurchasePrice || purchasePrice
	if (type === 'item') {
		return Number(signlePrice) > Number((purchasePrice - 0 + 3).toFixed(2))
	} else {
		return false
	}
}
...
function changeList() {
	const { productList } = props.dataList.data
			...
			if (item.skuList.length > 0) {
				item.skuList.forEach(a => {
					...
					if (
						...
						a.signlePrice =
							a.price || a.lastPurchasePrice || a.purchasePrice || 0
					}
...
function colSpanMethond({ row, rowIndex, columnIndex }) {
	...
	if (row.type === 'itemCh' && columnIndex !== 8) {
		return { rowspan: 1, colspan: 0 }
	}
	if (row.type === 'frCh') {
		if (columnIndex === 1) {
			return { rowspan: 4, colspan: 8 }
		} else {
			return { rowspan: 0, colspan: 0 }
		}
	}
	if (row.type === 'ch' && columnIndex !== 8) {
		return { rowspan: 0, colspan: 0 }
	}
}
...
function createMaxPrice(row) {
	...
	if (row.partnerType === 2)
	return 9999999.99
	// 1688 价格最高9999999.99
	if (Number((a.purchasePrice - 0 + 3).toFixed(2)) < 9999999.99) {
		return Number((a.purchasePrice - 0 + 3).toFixed(2))
	} else {
		return 9999999.99
	}
} // 最大单价
...
function computedTotal(row) {
	...
	fa.skuList?.map(a => {
		const { signlePrice, quantity } = a
		let price = Number(signlePrice) * quantity
		total += price || 0
	})
	let taxPrice = 0
...
function del(row) {
	const { id } = row
	if(data.selectedList.length && data.selectedList.some(val=>val.id === id)){
		data.selectedList = data.selectedList.filter(v=>v.id != id)
		if(!data.selectedList.length && data.tableCheckdAll) {
			data.tableCheckdAll = false
			data.tableIndeter = false
		}
	}
	let key = data.list.findIndex(v => v.id === id && v.type === 'item')
	if (key !== -1) {
		data.list.splice(key, 3)
	}
	data.list.forEach((item, index) => {
		if (item.type === 'fa') {
			let itemKey = item.skuList.findIndex(v => v.id === id)
			if (itemKey !== -1) item.skuList.splice(itemKey, 1)
			let hasCh = data.list.some(
				v => v.type === 'item' && v.parentId == item.id
			)
			if (!hasCh) {
				data.list.splice(index, 1)
				let frChKey = data.list.findIndex(
					v => v.id === item.id && v.type === 'frCh'
				)
				if (frChKey !== -1) data.list.splice(frChKey, 4)
			}
		}
	}) // 若skuList全部删除,则删除其父级和 底下form表单部分

	xtableRef.value.loadData(data.list)
} // 删除单个
...
const submit =  _debounce(async (status)=>{
	...
	// let formR = data.formRef?.filter(v => v)
	let promise = []
	...
	if(a.toDeptId && a.toDeptId?.length>0){
		...
		if (fa) {
			const { skuList } = fa
			skuList.forEach(v => {
				const { id, skuId, signlePrice, quantity, pairInfo, link,thirdPlatformSupplierId,goodAttribute } = v
				let sellerUserId
				if(pairInfo) sellerUserId = JSON.parse(pairInfo).sellerUserId
				let b = {
					id,
					skuId,
					purchasePrice: String(signlePrice),
					quantity,
					pairInfo: thirdPlatformSupplierId === sellerUserId ? pairInfo :null,
					// pairInfo,
					link,
					goodAttribute
				}
	...
	function back(res) {
		if (res.code === 200) {
			if (res.data.productList?.length > 0) {
				...
				// data.list = clearTree(res.data.productList)
			} else {
...
function editBack(val){
	...
	if(!val) return false
	data.list.forEach(v=>{
			if(v.type === 'item' && data.selectedList.some(val=>val.id === v.id)){
				if(val.goodAttributeChange) v.goodAttribute = val.goodAttribute
				if(v.partnerType !== 2 && v.purchasePrice -0 + 3 < val.signlePrice){
					if(val.signlePriceChange) v.signlePrice = (v.purchasePrice -0 + 3).toFixed(2)
				}else{
					if(val.signlePriceChange) v.signlePrice = val.signlePrice  + ''
				}
...
function handleCheckedRowAll(row,val){
	console.log('list+>',row,data.list)
	let checkedNum = 0
	data.list.forEach(v=>{
