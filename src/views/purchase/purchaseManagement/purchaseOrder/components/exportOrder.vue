        ...
        <el-tab-pane label="自定义导出" name="3">
          ...
          <el-form :model="form" :rules="rules" ref="formRef2">
            ...
            <el-form-item prop="purchaseNum" style="margin-top: 10px;">
              <el-input type="textarea" placeholder="请输入订单号" v-model="form.purchaseNum" show-word-limit clearable />
            </el-form-item>
          </el-form>
        </el-tab-pane>
      </el-tabs>
...
} from '../../purchaseReconciliation/api/api'
import { allPurchaseExport } from "@/api/purchase/purchase.js"
import draggable from 'vuedraggable'
import progressDialog from '../../purchaseReconciliation/components/progress'
import { progressInfo } from '@/api/basic.js'
import { purchaseProgress, queryPurchaseOrder } from '@/api/purchase/purchase.js'
...
export default {
  ...
  setup(props, context) {
    const rules = {
      ...
      purchaseNum: [{ required: true, message: '请输入订单号', trigger: 'blur' }],
    ...
    onMounted(() => {
      queryPurchaseOrder().then(res => { //选项
        if (res.code == 200) {
          state.list = res.data.purchase.concat(res.data.supplier)
        }
      })

      // console.log(props.status, '状态')
      queryExpotInfos({ type: 5 }).then(res => {//模版
    ...
    function exportHandle() {
      let bole = true
      if (state.form.type == 1 && state.form.subType == 2 && props.selectionArr.length == 0) {
      ...
      //查询条件
      if (state.form.type == 1 && state.form.subType == 1) {
				// const timeStart_timeEnd =  props.searchForm.timeStart_timeEnd
        // props.searchForm.timeStart = timeStart_timeEnd.length ? timeStart_timeEnd[0] : ""
        // props.searchForm.timeEnd = timeStart_timeEnd.length ? timeStart_timeEnd[1] + ' 23:59:59' : ""
        obj.orderDTO = deepCopy(props.searchForm)
      }
      //value值
      if (state.form.type == 1 && state.form.subType == 2) {
        obj.value = props.selectionArr.map(item => item.id).join(',')
      ...
      allPurchaseExport(obj).then(res => {
        downDetail(res.data)
      })
    }
    ...
    function selectChange(e) {
      ...
      let model = [],
        mainCheckKey = e.split('|').map(item => item.split(':')[0]) // 备选选择框主键的集合
      e.split('|').forEach(item => {
        let s = item.split(':')
        model = model.concat(s[1].split(','))
      })
      // 根据主键字段的顺序将待选数据按照后台配置顺序展示
      state.list = mainCheckKey.map(v => state.list.find(item => item.nameEn === v))
      // 确定三级选择框的选择状态
      data.forEach(item => item.checked = !!model.includes(item.nameEn))
      state.list.forEach(item => {
