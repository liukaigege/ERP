<template>
	<el-dialog :title="type == 0 ? '添加备注' : '编辑备注'" v-model="ifShow" :width="640" custom-class="myDialog" :close-on-click-modal="false" :destroy-on-close="true" @close="close">
...
import { remarkDetails } from '@/api/purchase/purchase.js'
	export default {
		props: {
      orderIds: {
        type: String,
      ...
      remarkId: {
		...
		setup(props, context) {
			let state = reactive({
				...
				remakeOptions: [],
				form: {
          id: props.type == 0 ? null : props.remarkId,
					typeId: null,
					content: '',
					orderIds: props.type == 0 ? [props.orderIds] : null
				},
        loading: false
			})
			let rules = {
      ...
      let getRemarkDetail = () => {
        remarkDetails({ id: props.remarkId }).then(res => {
          if (res.code == 200) {
            state.form.typeId = res.data.typeId
        ...
        if (props.type == 1)  getRemarkDetail()
			})
