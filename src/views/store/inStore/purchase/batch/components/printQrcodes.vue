 ...
 * @version:
 * @Author: 韦剑
 * @Date: 2021-07-28 19:08:39
 * @LastEditors: 韦剑
 * @LastEditTime: 2021-08-22 21:57:42
-->
<template>
          ...
          <div class="infoList">
            <div class="item" v-for="(item, index) in goodsInfo" :key="item.uuId">
              ...
              <div class="right">
                ...
                <el-input-number size="small" :precision="0" v-model="item.originQuantity" style="width: 110px" controls-position="right" :min="0" :max="item.maxNum" @blur="printNumBlur(item.originQuantity, index)"></el-input-number>
...
export default {
  ...
  setup(props, context ) {
      ...
      state.goodsInfo.forEach(item => {
        if (item.originQuantity !== 0) {
          let obj = {
            ...
            num: item.originQuantity,
            qrcode: '',
            skuValue: item.speValEn
          }
          arr.push(obj)
        }
      })
    ...
    let printNumBlur = (value, index) => {
      if (value == undefined) state.goodsInfo[index].originQuantity = 0
    }
