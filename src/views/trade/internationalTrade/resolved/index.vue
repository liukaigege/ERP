  ...
  <div>
      ...
      <div class="form">
        <el-form :inline="true" :model="form" class="demo-form-inline">
          <div>
            ...
            <el-form-item label="纠纷结果：">
              <el-select
                v-model="form.outcomeCode"
    ...
    <div class="table">
      ...
      >
        ...
        <el-table-column prop="outcomeCode" label="纠纷结果" align="center"
          ><template #default="{ row }">
        ...
        <!-- <el-table-column prop="caseType" label="纠纷状态" align="center" /> -->
        <el-table-column
        ...
        <el-table-column prop="address" label="时间" align="center" width="200"
          ><template #default="{ row }">
...
const erpPageBack = v => {}
const init = () => {
  tradestatesolve(state.form).then(res => {
    if (res.code == 200) {
...
function outcomeCode(params) {
  switch (params) {
    case 'RESOLVED_BUYER_FAVOUR':
      return '争议以对客户有利的方式解决'
    case 'RESOLVED_SELLER_FAVOUR':
      return '争端以有利于商家的方式解决'
    case 'RESOLVED_WITH_PAYOUT':
      return 'PayPal 为商家或客户提供了保护，案件得到解决'
    ...
    case 'DENIED':
      return '贝宝否认了这一争议'
    case 'NONE':
      return '为同一个交易 ID 创建了一个争议，之前的争议在没有任何决定的情况下被关闭'
  }
}
